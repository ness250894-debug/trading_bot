import duckdb
import json
from datetime import datetime
import os

# --- INSTRUCTIONS FOR AWS ---
# 1. Ensure 'duckdb' is installed:
#    pip install duckdb
# 2. Run this script from the project root:
#    python backend/seed_plans_standalone.py

# Path to your database file
# Assuming standard layout: project_root/data/trading_bot.duckdb
DB_PATH = 'data/trading_bot.duckdb'

def seed_plans():
    if not os.path.exists(DB_PATH):
        print(f"❌ Error: Database not found at {DB_PATH}")
        print("Please run this script from the project root directory.")
        return

    print(f"--- Seeding Plans into {DB_PATH} ---")
    
    # Connect directly using duckdb
    conn = duckdb.connect(DB_PATH)
    
    plans = [
         # Basic
        {
            "id": "basic_monthly",
            "name": "Basic (Monthly)",
            "price": 19.0,
            "currency": "USD",
            "duration_days": 30,
            "features": [
                "live_trading", 
                "max_bots_3", 
                "standard_strategies", 
                "sentiment_standard", 
                "realtime_data"
            ]
        },
        {
            "id": "basic_yearly",
            "name": "Basic (Yearly)",
            "price": 190.0,
            "currency": "USD",
            "duration_days": 365,
            "features": [
                "live_trading", 
                "max_bots_3", 
                "standard_strategies", 
                "sentiment_standard", 
                "realtime_data"
            ]
        },
        # Pro
        {
            "id": "pro_monthly",
            "name": "Pro (Monthly)",
            "price": 49.0,
            "currency": "USD",
            "duration_days": 30,
            "features": [
                "live_trading", 
                "max_bots_10", 
                "visual_builder", 
                "backtesting", 
                "optimization_standard", 
                "sentiment_advanced", 
                "quick_scalp"
            ]
        },
        {
            "id": "pro_yearly",
            "name": "Pro (Yearly)",
            "price": 490.0,
            "currency": "USD",
            "duration_days": 365,
            "features": [
                "live_trading", 
                "max_bots_10", 
                "visual_builder", 
                "backtesting", 
                "optimization_standard", 
                "sentiment_advanced", 
                "quick_scalp"
            ]
        },
        # Elite
        {
            "id": "elite_monthly",
            "name": "Elite (Monthly)",
            "price": 99.0,
            "currency": "USD",
            "duration_days": 30,
            "features": [
                "live_trading", 
                "max_bots_unlimited", 
                "visual_builder", 
                "backtesting", 
                "optimization_ultimate", 
                "sentiment_advanced", 
                "quick_scalp",
                "priority_support",
                "unlimited_exchanges"
            ]
        },
        {
            "id": "elite_yearly",
            "name": "Elite (Yearly)",
            "price": 990.0,
            "currency": "USD",
            "duration_days": 365,
            "features": [
                "live_trading", 
                "max_bots_unlimited", 
                "visual_builder", 
                "backtesting", 
                "optimization_ultimate", 
                "sentiment_advanced", 
                "quick_scalp",
                "priority_support",
                "unlimited_exchanges"
            ]
        }
    ]

    for plan in plans:
        # Check if exists
        # DuckDB usually needs parameter markers as ? or $1 depending on context, python client uses ?
        existing = conn.execute("SELECT 1 FROM plans WHERE id = ?", [plan['id']]).fetchone()
        
        now = datetime.now()
        
        if existing:
            print(f"Updating plan: {plan['name']}")
            conn.execute("""
                UPDATE plans 
                SET name = ?, price = ?, currency = ?, duration_days = ?, features = ?, updated_at = ?
                WHERE id = ?
            """, [
                plan['name'], 
                plan['price'], 
                plan['currency'], 
                plan['duration_days'], 
                json.dumps(plan['features']), 
                now, 
                plan['id']
            ])
        else:
            print(f"Creating plan: {plan['name']}")
            conn.execute("""
                INSERT INTO plans (id, name, price, currency, duration_days, features, is_active, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, [
                plan['id'],
                plan['name'],
                plan['price'],
                plan['currency'],
                plan['duration_days'],
                json.dumps(plan['features']),
                True,
                now,
                now
            ])
    
    # Close connection
    conn.close()
    print("✅ Plans seeded successfully!")

if __name__ == "__main__":
    seed_plans()

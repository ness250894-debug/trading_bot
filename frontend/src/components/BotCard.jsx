    ...
    return (
        <div className={`glass p-4 rounded-xl border transition-all ${isSelected ? 'border-primary/50 bg-primary/5' : 'border-white/5'}`}>
            ...
            <div className="grid grid-cols-2 gap-2 text-sm text-muted-foreground mb-4 bg-white/5 p-3 rounded-lg">
                ...
                <div>TP/SL: <span className="text-green-400 font-mono">{bot.take_profit_pct ? (bot.take_profit_pct * 100).toFixed(1) + '%' : '-'}</span> / <span className="text-red-400 font-mono">{bot.stop_loss_pct ? (bot.stop_loss_pct * 100).toFixed(1) + '%' : '-'}</span></div>

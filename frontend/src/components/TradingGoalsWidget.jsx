...
import { Target, Plus, Edit2, Trash2, Check, X, Calendar, Archive, LayoutList } from 'lucide-react';
...
export default function TradingGoalsWidget() {
    ...
    return (
        <div className="glass rounded-2xl p-6 border-l-4 border-l-primary/50">
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className={`p-2 rounded-lg transition-colors ${showArchived ? 'bg-primary/20 text-primary' : 'bg-white/5 text-muted-foreground hover:bg-white/10'
                            }`}
                        title={showArchived ? "Show Active Goals" : "Show Archived Goals"}
                    >
                        {showArchived ? <LayoutList size={20} /> : <Archive size={20} />}
                    </button>
                    <button
            ...
            {
                goals.filter(g => showArchived ? g.is_completed : !g.is_completed).length === 0 ? (
                    <div className="text-center py-12 text-muted-foreground">
                        {showArchived ? (
                            ...
                            <>
                                ...
                                <p>No active trading goals. Create one to start tracking your progress!</p>
                            </>
                        )}
                    </div>
                ) : (
                    <div className="space-y-4">
                        {goals.filter(g => showArchived ? g.is_completed : !g.is_completed).map((goal) => {
